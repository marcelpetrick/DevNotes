...
* the semis are focusing a lot on AI: 90% of a room full of engineers will say: nvidia ..
...
* EI has royalty-free business model, so no impact on the BOM cost, total explain-ability, no black boxes; ML solutions in minutes, not months
...
* good data, good modeling, shitty data, shitty modeling ..
...
### examples
* RAM-1; power line monitoring application; 
* nordic Thingy:53 is already flashed with EI firmware flashed; you have an app as well; via the app you can sample, build and deploy the models
* brainchip: as neuromorphic IP, Akida platform; will just sample necessary sensory inputs instead of everything
* cortex m7 is coming, m55 plus u55 uplift up to 55x faster and 25x less energy for voice sampling
* Alif E will be 22x faster than a RPi with image detection
* Oura (ring) uses also the EI platform to improve their algorithm
* packaging handling: how to make sure packages are handled with care?
* also for non invasive glucose measurement: know labs (apple said later they want to do the same)
...
* 2 years ago they created HW-prototypes, tried to integrate the cloud, because it was crushing pressure
...
* this was not the focus: so the could not become hardware and ml-experts at the same time
...
* on device processing makes it possible to process more data and then send just the result
...
* since the transparent case of the serial.connector does block my second usb-port, I can't continue with the demo :/
