...
![](img02.png)
* module license is also needed to be added

![](img03.png)
![](img04.png)
* kernel offers different frameworks to help: like regmap and pinctrl, gpiod, ..
* many types of hardware are interrupt-driven and will signal if they have something to say
![](img05.png)
* linux driver model
![](img06.png)
...
* initialized, registered with matching framework, then matching to, populates the device
![](img08.png)
* platform devices
![](img09.png)
![](img10.png)
...
![](img25.png)
![](img26.png)
* mainlining: **pro: if they change an API, then they must also change your committed module - no work for you! :)**
...
A:
    A phandle is an entry in the devicetree source which refers to another node or property elsewhere in the same devicetree 
```
```
Q:
...
A:
    There are a few options, Either using 'gdb' and 'kgdb' or making use of external JTAG debuggers. If you can run development in a virtualised environment, such as qemu it is possible to debug the kernel with GDB directly, as if it were connected over jtag. The specifics of debugging with JTAG will depend on which jtag device you have. 
```
```
Q:
...
A:
    It was developed over many iterations of the kernel, which is constantly being updated and improved.
```
```
Q:
...
A:
    All code can have vulnerabilities, but one of the key aims of the kernel drivers is to reduce attack vectors. The linux kernel has privileged access to the hardware, and so it is important that the kernel-userspace interfaces are validated. There are many projects that test these interfaces and perform 'fuzzing' to check the security. Bugs identified are fixed and backported in the kernel stable tree.
```
```
Q:
    what about ioctl() ?, in the past I use it a lot. Is used also in more modern kernel version ? 
A:
    ioctls are a fundamental mechanism which allow drivers to provide an interface to user space which doesn't have to fit in with any of the standard file operations read, write etc. It is very common in device drivers.
```
```
Q:
    are unlocked_ioctl the same as private ioctl's?
A:
    unlocked_ioctl is the unlocked version of ioctl which was part of removing the 'big kernel lock' to protect against bugs when running on SMP systems. I believe this is a long handled task now so unlocked_ioctls are the normal path to take.
```
```
Q:
...
```
Q:
    how do we determine the clock for I2c? or we don't need to determine it?
A:
...
Q:
    Rust OOP Language? 
A:
    Rust is an object orientated language in that it can operate with methods and interfaces on structs and enums ... but it's a bit different from C and C++. It has aspects that make it interesting to consider for system level programming, and is why it has been considered for use within the linux kernel.
```
```
Q:
