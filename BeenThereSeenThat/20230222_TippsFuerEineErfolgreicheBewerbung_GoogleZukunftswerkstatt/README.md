# 20230222 - Tipps für eine erfolgreiche Bewerbung - Google Zukunftswerkstatt
* Sonja Karmann ist die Trainerin; Thema Kommunikation; NLP Master
* zukunftswerkstatt.de oder goo.gle/zw-videos
* Zusammenarbeit mit XING/NewWorkSE (Kununu, honeypot, hallo Freelancer, Internations..)
* Möglichkeit der Zertifikaterlangung

## Agenda
* Die eigenen Stärken und Fähigkeiten (er-)kennen
* Den Lebenslauf erstellen
* Das Anschreiben gestalten
* In Online-Bewerbungsgesprächen überzeugen

* weltweit planen 82% der Unternehmen Neueinstellungen (Monster, 2021, weltweit)

## Stärken, Fähigkeiten und Erfahrung
* Stärken: vorteilhafte Eigenschaften, die jemand natürlicher Weise besitzt (Soft skills)
* Fähigkeiten: Können erlernt bzw. beigebracht werden
* Erfahrung: ergibt sich nur über die Zeit, z.b. in einer bestimmten Position oder in einem Projekt

* note: alle drei unterschiedlich
* keywords für die Stärken: enthusiastisch, lösungsorientiert, zuverlässig, fokussiert, hilfsbereit, flexibel, dynamisch, empathisch, kommunikativ, motiviert, analytisch, leidenschaftlich, neugierig
* keywords, die man für die Fähigkeiten nutzen kann: HTML, Moderation, Aktienhandel, Grafikdesign, python, Rechnungswesen, ..

* Übung:
  * eigene stärken: fokussiert, hilfsbereit, flexibel, dynamisch, kommunikativ, analytisch, neugierig, lösungsorientiert, kundenorientiert, Lernbereitschaft
  * Fähigkeiten: Deutsch, Englisch, Mandarin, C++, Qt, Python, Bildverarbeitung, Embedded SW-development, Wandern, Kochen - lieber ersteinmal initial alles aufschreiben, was passt, dann filtern
  * Erfahrung: Software-Entwicklung, Teamleading, Organisation komplexer Projekte - Online-Kurse, Weiterbildungen, Zertifikate, Ehrenamtliches Engagement, Hobbys, Privatleben; Führungserfahrung aus Gruppenleitungen (Feuerwehr, Pfadfinder), Auslandsjahr, Familienpflege, ..
  
### Die eigenen Stärken und Fähigkeiten finden:
* positives Feedback für mich finden: abgeleitet: meine Stärken, wie kann ich sie noch besser nutzen? Meine Größten Erfolge, was habe ich daraus gelernt
  * mein "one thing" - was man mitbringt, was keiner mitbringt? hervorstechen?
* negatives Feedback für mich finden: abgeleitet: Entwicklungsfelder; Was ich in anderen suche? Auch als Beispiel nutzen für "Was sind Schwächen, so dass man diese mit einem Beispiel unterlegen kann."
  * meine größten Misserfolge, Was ich daraus gelernt habe

### Was sind ihre Top 3 Stärken?
* Warum haben sie genau die drei Stärken ausgewählt?
* Was können Sie mit diesem Stärken tun?
* Wie könnten Sie die Stärken im Lebenslauf und beruflichen Netzwerken herausstellen? ansprechendes Design für die Bewerbung, falls man kreativ ist; ordentliche Leute sollten keinerlei Fehler im Schreiben haben

https://goo.gle/personenmarke - todo check this

### Für welches Unternehmen möchten Sie arbeiten?
* Gibt es bestimmte Branchen
* Organisationsformen
* Unternehmen
* Marken oder Produkte für bzw. mit denen Sie gerne arbeiten möchten?

* auch nützlich um sich vorzubereiten für die Frage: Warum haben sie sich gerade hier beworben?
* die Stellenbeschreibung nicht als Checkliste mit 100% Haken sehen: sondern als Wunschvorstellung des Arbeitgebers

## Lebenslauf
* schriftliche Darstellung der wichtigsten Daten und Ereignisse des eigenen Lebens
* Der Lebenslauf ist der erste Kontakt mit dem Unternehmen:
  * Zusammenfassung der Erfahrung
  * Zielorientiert und relevant
  * authentisch und ehrlich
* welche Aspekte gehören in den Lebenslauf
  * Kontaktdaten (dort auch!)
  * Kurzprofil (relativ neu)
  * Erfahrungen
  * Ausbildung
  * Fähigkeiten und Stärken
  * Kontaktdaten: Name, Anschrift (wichtig wegen Fahrkosten & Erstattung), Telefon, E-Mail-Adresse, Geburtsdatum
## Kurz-Profil
* Wer sind sie?
* Was bringen sie für die ausgeschriebene Stelle mit?
* Was macht Sie besonders?
* `Elevator pitch` über sich selbst - 2 bis 3 Sätze, wer man ist
## Erfahrungen
* die aktuelle Erfahrung ganz oben
* Unternehmen
* Stellenbezeichnung
* Von wann bis wann waren sie tätig?
* Schlüsselprojekte und erreichte Ergebnisse
* Ehrenamtliches Engagement, u.ä. inkludieren, wenn es für die Stelle relevant ist

## Ausbildung
* Berufliche und akademische Ausbildung
* die aktuellste Erfahrung ganz oben
* Qualifikation/Abschluss/Zertifikat
* Institution
* Daten/Dauer der Ausbildung
* gegebenenfalls Schlüsselpunkte/Schwerpunkte

## Fähigkeiten und Stärken
* Welche Fähigkeiten und Stärken bringen sie mit?
* wie stark sind die Fähigkeiten ausgebildet (Balken oder Zahl als Bewertung des eigenen Levels)
* Welches Level bringen Sie mit?
* Bezug zu der ausgeschriebenen Stelle herstellen

## Lücken im Lebenslauf
* Ehrlichkeit zahlt sich aus
* Lücken erklären und aufzeigen was Sie in der Zeit gemacht haben
* Haben Sie z.B. neue Fähigkeiten erworben oder konnten Stärken ausbauen?
* Lügen gegebenenfalls als Kündigungsgrund

## Tools zur Erstellung des Lebenslaufes
* lebenslauf.com als Tool
* ob man ein Photo hinzu packt, ist individuell; weglassen passt
* Farben gegebenenfalls
## Welche Aspekte gehören ins Anschreiben
* Adresse und Kontaktdaten
* Anrede
* Einleitung
* Zusammenfassung Ihres Profils
* Lösung für die Herausforderung des Teams/des Unternehmens
* Relevante Stärken und Fähigkeiten
* Verabschiedung

* gibt auch wieder ein Tool hierfür
* wenn man sich vorher kannte, dann direkt die Person ansprechen, sonst eher förmlich
* angeben, wo man die Stelle gefunden hat
* im Mittelteil kurz die aktuelle Position, was man dort macht, etc.
* weshalb ist man die richtige Person, inwiefern kann man sich einbringen?
* Anschreiben wird schon empfohlen, nicht nur für Texter oder Social Media Manager ..

## Angeblich informieren sich 2 von 3 Personalern online über die Bewerberinnen
* andererseits gibt es die Rückmeldung, dass es doch nicht so verbreitet ist
* lieber selbst vorher googlen

## Tipps für das Online-Bewerbungsgespräch
* stabile Internetverbindung: das A und O für ein erfolgreiches Bewerbungsgespräch online
* software: vorab installieren und testen, auf professionellen Benutzernamen achten
* Hardware: Mikrofon und Kamera testen
* Kamera ausrichten: auf Augenhöhe, sie sollte möglichst zentriert im Bild sichtbar sein
* Beleuchtung: weiches Licht von vorn, kein Licht von hinten
* Kleidung: passend zum Unternehmen und zur Stelle
* Muster wirken schnell verstörend
* auf Accessoires verzichten, die Krach machen
* Körperhaltung: bequem, aber aufrecht
* Vorbereitung: Stellenausschreibung nochmal durchgehen, Infos rund um das Unternehmen, die Stelle recherchieren, Ihre Unterlagen griffbereit haben, Fragen vorbereiten
* Test-Interview: vorab die Interviewsituation mit Freunden üben
