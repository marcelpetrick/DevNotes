...
* cybersecurity a language problem
* security specific copilots
so what? how we can deeply integrate LMs
three layers of stack where language models work
what to take away
...
* any sufficiently represented model would suffer from vocabulary explosion for available memory
* LSTM: lacked enough context for complex associations
...
* high utility and success when the target domain is small and limited in scope; small context window is adequate to give salient results
relative number of tokens is small or can be constrained
img02
* example applications for CS include:
 * log parsing (cyBERT)
 * root cause analysis
 * basic anomaly detection
* architecture fails, when even more additional context/tokens needed or when domain becomes large

## copilots
* "this talk is not an LLM talk"
* this is about how we can harness their power
* how to solve real world cyber security problems
* three critical issues are addressed and are the easy button of LLM integration:
...
* a copilot can pull together a large amount of information, which allows better incident response
* also: upskilling existing staff
img00
* key challenge for security copilots
...
# generative AI knowledge BASE chatbot (based on RAG)
todo: image - rag
...
## CVE exploitability and vulnerability analysis
* dramatically improve vulnerability defense with generative AI
* it matches a human analyst with 85% accuracy: but with faster time, not bad for the first version; 4 times faster
* reduction in CVE-noise
todo: image
* CVE exploitability using the LLM engine
* is CVE-2022-42890 exploitable in morpheus v23.07.00a?
todo: image
* an initial query goes to the system, then generates information, ... which culminates in a human readable conclusion
  * see the evaluation: run 0
  todo: image: of the run
...
* running these requirements. action: internet lookup; back to the top - task generator: search java runtime environment using sbom QA system ..
* .. presence does not always mean exploitability!
* traditional RAG architecture:
todo: image
* tweaked version: need an always on service and as many reusable components as possible
todo: image
* langchain or llama index can be used directly in that design
...
todo image
todo image2
* cyberGPT training times: 2 GPU hours for GPT-2 model with character level tokenization
* choices for the tokenizer
  * character level tokenizer; off the shelf tokenizer; train custom tokenizer
todo: image tokenizer
* demo: cyberGPT-demo ..
* also the possibility to create high quality synthetic data
...
### how can models be trained for spear phishing campaigns
* low volume, high success
* synthetic data generation for spear phishing: default models trained for off-the-shelf intents
